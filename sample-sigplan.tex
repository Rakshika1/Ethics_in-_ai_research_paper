...
%% used to denote shared contribution to the research.\author{Rakshika Sharma}
\author{Rakshika Sharma} % (removed any invisible Unicode character)
...
\centering
\begin{tabular}{c c}
\textbf{Priya Sharma} & \textbf{Priya Devi} \\
\includegraphics[width=0.4\linewidth]{Experiment-01/sharma.png} &
\includegraphics[width=0.4\linewidth]{Experiment-01/devi.png} \\
\end{tabular}
\caption{Outputs generated using identical prompts with different identity-linked names.}
\label{fig:identity}
\end{figure}

Notably, these variations emerge despite no explicit differences in the prompt, suggesting that identity-linked names influence how individuals are visually and contextually represented.
\\
To further highlight these differences, we present a brief comparison in Table 1.
\begin{table}[H]
\centering
\begin{tabular}{l p{2.5cm} p{2.5cm}}
\hline
\textbf{Aspect} & \textbf{Priya Sharma} & \textbf{Priya Devi} \\
\hline
Visual Markers & Minimal & Bindi present \\
Styling & Modern corporate & Slightly traditional \\
Accessories & Smartwatch (modern wearable) & Analog watch (traditional style) \\
Representation & Neutral-professional & Culturally contextualized \\
\hline
\end{tabular}
\caption{Subtle differences in representation across identity-linked names.}
\label{tab:identity}
\end{table}

Even in this simple example, consistent differences emerge across multiple aspects of representation, suggesting the influence of implicit identity associations.\\
In this paper, we investigate how identity-linked names influence generative AI outputs across multiple tasks, including text generation, image generation, and decision-making scenarios. To achieve this, we employ a minimalist experimental design wherein the prompt remains constant while only the name is varied, allowing us to directly observe the impact of identity signals on the resulting output. Our experiments demonstrate that names do not merely induce minor or superficial changes; rather, they drive large-scale transformations in the output. In many instances, the AI ​​system constructs a completely distinct social reality based solely on the name provided. (blank line)
We term this phenomenon "Identity Collapse"—a process wherein a single identity cue triggers a cascade of assumptions that ultimately shape the entirety of the generated output. These findings highlight that... It is not limited merely to representation; rather, AI systems construct diverse lived realities based on identity. Therefore, this paper argues that existing algorithmic fairness approaches are insufficient and that a new, context-aware framework is required—one that takes into account identity-driven generation.
...
We adopt a minimal-pair design inspired by prior bias evaluation frameworks \cite{nangia2020crows, sheng2019woman}. In each experiment, two or more prompts are constructed such that all elements remain constant except for the name of the individual.
...
We conduct a total of thirteen experiments to evaluate how identity signals embedded in names influence generative AI outputs. All experiments follow a minimal-pair design, where prompts remain identical except for the name or identity-linked attribute. 
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-01/sharma.png}
    \caption*{\textbf{Priya Sharma}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-01/devi.png}
    \caption*{\textbf{Priya Devi}}
  \end{minipage}
  \caption{Identical prompts produce culturally distinct outputs
           based solely on the name.}
  \label{fig:priya-exp1}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment_02/Street2.png}
    \caption*{\textbf{Priya Devi}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment_02/Street.png}
    \caption*{\textbf{Priya Sharma}}
  \end{minipage}
  \caption{Identical prompts produce culturally and socioeconomically
           distinct outputs based solely on the name. \textit{Priya Devi}
           is placed in a traditional local market; \textit{Priya Sharma}
           in a modern commercial street with international brands.}
  \label{fig:priya-exp2}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-03/Rahul-Sharma.png}
    \caption*{\textbf{Rahul Sharma}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-03/Rahul-Jatav.png}
    \caption*{\textbf{Rahul Jatav}}
  \end{minipage}
  \caption{Identical prompts produce geographically distinct outputs.
           \textit{Rahul Sharma} appears in a Western metropolis;
           \textit{Rahul Jatav} in a local Indian market street.}
  \label{fig:rahul-exp3}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-04/fatima_sharma.png}
    \caption*{\textbf{Fatima Sharma}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-04/khan.png}
    \caption*{\textbf{Fatima Khan}}
  \end{minipage}
  \caption{Identical prompts produce religiously distinct outputs
           based solely on the surname. \textit{Fatima Sharma} is
           depicted without any religious markers; \textit{Fatima Khan}
           is automatically assigned a hijab.}
  \label{fig:fatima-exp4}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-05/dhobi.png}
    \caption*{\textbf{Ramavatar Dhobi}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-05/sharma.png}
    \caption*{\textbf{Ramavatar Sharma}}
  \end{minipage}
  \caption{Identical prompts produce dramatically different
           celebratory environments based solely on the
           caste-linked surname. \textit{Ramavatar Dhobi} is placed
           in a cramped, worn room; \textit{Ramavatar Sharma}
           in a spacious, well-decorated party setting.}
  \label{fig:birthday-exp5}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_sharma.png}
    \caption*{\textbf{Tanu Sharma}}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_valmiki.png}
    \caption*{\textbf{Tanu Valmiki}}
  \end{minipage}
  \caption{Identical prompts specifying only name and city produce
           entirely different versions of Mumbai. \textit{Tanu Sharma}
           appears before the UNESCO-listed Chhatrapati Shivaji Maharaj
           Terminus on a clean open road; \textit{Tanu Valmiki} appears
           in a narrow decaying lane behind an unprompted rusted iron gate.}
  \label{fig:tanu-exp6}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_sharma_2.png}
    \caption*{\textbf{Tanu Sharma} \\ \small(Brahmin / Upper Caste)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_meena.png}
    \caption*{\textbf{Tanu Meena} \\ \small(Scheduled Tribe)}
  \end{minipage}
  \vspace{0.4em}
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_yadav.png}
    \caption*{\textbf{Tanu Yadav} \\ \small(OBC)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{Experiment-06/Tanu_meghwal.png}
    \caption*{\textbf{Tanu Meghwal} \\ \small(Scheduled Caste / Dalit)}
  \end{minipage}
  \caption{Extended caste-spectrum comparison using identical prompts.
           The same city (Mumbai) is reconstructed into progressively
           more marginalised environments as the caste-linked surname
           descends the social hierarchy. \textit{Tanu Sharma} occupies
           an aspirational open waterfront; \textit{Tanu Meena} is placed
           on a public bus with Marine Drive visible through the window;
           \textit{Tanu Yadav} is seated at a roadside chai stall;
           \textit{Tanu Meghwal} is placed in a busy bazaar lane
           surrounded by auto-rickshaws and local market signage.}
  \label{fig:tanu-spectrum}
\end{figure}
...
\textbf{Case 1: Candidate from Barmer} \\
\includegraphics[width=0.9\linewidth]{exper11/exper12/Picture1.png} \\[6pt]

\textbf{Model Output (Barmer)} \\
\includegraphics[width=0.9\linewidth]{exper11/exper12/Picture2.png} \\[6pt]

\textbf{Case 2: Candidate from Bangalore} \\
\includegraphics[width=0.9\linewidth]{exper11/exper12/Picture3.png} \\[6pt]

\textbf{Model Output (Bangalore)} \\
\includegraphics[width=0.9\linewidth]{exper11/exper12/Picture4.png} \\

\end{tabular}

\caption{Experiment 9: Geographic proxy bias in shortlisting decisions. Despite identical qualifications, the candidate is rejected when associated with Barmer but shortlisted when associated with Bangalore, indicating location-based bias.}
...
\textbf{Input Candidates and Prompt} \\
\includegraphics[width=0.9\linewidth]{exper11/Picture1.png} \\[6pt]

\textbf{Candidate Details (Extended)} \\
\includegraphics[width=0.9\linewidth]{exper11/Picture2.png} \\[6pt]

\textbf{Generated Ranking Output} \\
\includegraphics[width=0.9\linewidth]{exper11/Picture3.png} \\[6pt]

\textbf{Model Reasoning} \\
\includegraphics[width=0.9\linewidth]{exper11/Picture4.png} \\

\end{tabular}

\caption{Experiment 11: Candidate ranking bias. Despite identical qualifications, candidates with dominant caste-associated surnames (e.g., Sharma) are ranked higher than those with marginalized caste-associated surnames (e.g., Paswan, Valmiki). The model explicitly attributes this to learned associations from historical data.}
...
\includegraphics[width=0.9\linewidth]{exper13/Picture1.png} \\[6pt]


\includegraphics[width=0.9\linewidth]{exper13/Picture2.png} \\[6pt]


\includegraphics[width=0.9\linewidth]{exper13/Picture3.png} \\[6pt]


\includegraphics[width=0.9\linewidth]{exper13/Picture4.png} \\

\end{tabular}

\caption{Experiment 12: Interview shortlisting bias. Despite identical qualifications, candidates receive different shortlisting outcomes based on identity-linked names, indicating bias in evaluation and selection decisions.}
...
\textbf{Version A: Hindu Crowd / Muslim Victims} \\
\includegraphics[width=0.82\linewidth,height=6.2cm,keepaspectratio=true]{exp15/exp13neww/pic1.png} \\[8pt]

\textbf{Version B: Muslim Crowd / Hindu Victims} \\
\includegraphics[width=0.82\linewidth,height=6.2cm,keepaspectratio=true]{exp15/exp13neww/pic2.png} \\[8pt]

\textbf{Headline and Tone Comparison} \\
\includegraphics[width=0.82\linewidth,height=6.2cm,keepaspectratio=true]{exp15/exp13neww/pic3.png} \\

\end{tabular}

\caption{Religious identity reversal produced significant differences in tone, headline framing, and threat narratives despite identical event facts.}
...
\textbf{Police Response and Accountability Comparison}

\vspace{0.25cm}

\includegraphics[width=\linewidth,height=8cm,keepaspectratio=true]{exp15/exp13neww/pic4.png}

\end{minipage}

\caption{Institutional response language varied across identity-swapped versions, with differential emphasis on probes, arrests, and security failure.}
...
\textbf{Generated Narrative (Overview)} \\
\includegraphics[width=0.9\linewidth]{exper11/exp14/Picture1.png} \\[6pt]

\textbf{Morning Routine Comparison} \\
\includegraphics[width=0.9\linewidth]{exper11/exp14/Picture2.png} \\[6pt]

\textbf{Workday Behavior Comparison} \\
\includegraphics[width=0.9\linewidth]{exper11/exp14/Picture3.png} \\[6pt]

\textbf{Evening Lifestyle Comparison} \\
\includegraphics[width=0.9\linewidth]{exper11/exp14/Picture4.png} \\

\end{tabular}

\caption{Narrative divergence across identity-linked names. Despite identical professional and socioeconomic conditions, the model constructs significantly different daily routines, behaviors, and life trajectories for each individual.}

\label{fig:expNarrative}
\end{figure}
...
frequently~\cite{bbq2021,decaste,indicasa}. The present findings
...
from Western fairness benchmarks~\cite{bbq2021}. These axes interact:
...
%  Paste this after your Analysis section in Overleaf
% ─────────────────────────────────────────────────────────────
...
\end{thebibliography}
\subsubsection*{Experiment 7: Caste-Based Social Preference Bias}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{Experiment-07/social_bias.png}
  \caption{When asked to choose a seat companion, the model
           consistently recommended upper-caste/merchant-associated
           names (Agarwal, Sharma) over OBC-associated (Yadav)
           and tribal-associated (Bhil) names, describing Yadav
           as ``unpredictable'' and Bhil as a ``wildcard.''}
  \label{fig:exp7}
\end{figure}
...
\subsubsection*{Experiment 8: Caste-Based Personality 
Trait Attribution}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{Experiment-08/trait_bias.png}
  \caption{The model assigned systematically different personality
           traits based solely on caste-linked surnames.
           Ashok Sharma received structured, stable, low-risk
           traits while Ashok Yadav received opportunistic,
           chaotic, and impulsive characterisations.}
  \label{fig:exp8}
\end{figure}
...
\subsubsection*{Experiment 9: Name Prestige Bias 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{Experiment-09/ramu_rana.png}
  \caption{Without any profession or class instruction,
           \textit{Ramu} is depicted as a rural labourer
           in an agricultural field while \textit{Rana Singh}
           is represented as an elite professional in a
           formal mahogany office --- based solely on
           name prestige.}
  \label{fig:exp9}
\end{figure}
